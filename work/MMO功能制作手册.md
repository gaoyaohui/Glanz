...
生效后的规则，所有开发流程依据新的规则进行。之前依据旧规则执行的功能，等待更新优化版本周期内，再统一进行规则翻新。
...
### SVN使用规则 ###
1. 在功能完成后体验时，若发现出现材质丢失情况，先进行revert，恢复到服务器上的内容再进行体验。<sup>12</sup>
2. 
### 服务器开发
1. 所有S\A级问题需要在提出后的2天内修复

### UI效果图制作
日常流程：
1. 功能会前，策划需要与UI制定具体的可实现的功能效果图以及UE方式。功能会前置<sup>9</sup>。
2. 功能会完成后，UI同步拆分功能效果图制作时间。
3. UI制作完毕后，美术主管效果确认
4. 跟进策划效果确认，并在禅道内完成验收修复任务<sup>13</sup>
5. 程序或策划将资源打图集，完成资源拼接。
6. 诗雨进行拼接后的NGUI二次校对。
7. 提交测试
...
4. 效果图制作时间应包含修改时间或调校完成时间<sup>13</sup>
...
2. 编写功能代码
3. 根据UI提供功能效果图以及图素拼接预制。
3. 与服务器联调，上传SVN（联调需要添加对应的联调任务）<sup>10</sup>
4. 所有S\A级问题需要在提出后的2天内修复
4. 进行客户端打包
5. 
...
c-**因UImain表格是所有人共用，因此使用UImain资源时，务必更新到最新，再进行提交。**<sup>11</sup>
...
7. 完成功能跟进，填写策划验收报告。
8. **所有验收内发现未完成的功能会被提交S\A级BUG，并标注类型为什么没实现。此类BUG需要在2天内完成**
...
10. 2018-2-9 斗法殿任务验收过程，连续三天眼验收。连续发现阻断性问题。发现服务器与客户端联调实际存在缺失，或者配合过程中没有那么默契
11. 2018-2-9 按钮特效二次丢失，查证后发现UImain没有更新到最新进行了替换，导致资源丢失
12. 2018-3-8  金角银角特效材质丢失——特效上传一次后解决。
13. 2018-3-24 个人空间效果图完成后，程序默认效果图内文本为最终文本，但实际是美术根据自身理解定义的名称，另外，效果图完成时间与实际审核完成时间之间还存在一定误差